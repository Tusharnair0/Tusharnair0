...
router.post("/items", async (req, res) => {
  const condition = req.body.items;

  try {
    const Products = await Product.find({ _id: { $in: condition } });
...
router.get("/:id", async (req, res) => {
  ...
  try {
    ...
    console.log(1);
    res.status(500).json({ success: false, message: "Interval cc server error" });
  }
});

router.get("/recommend/:q", verifyCustomerToken, async (req, res) => {
  const productId = req.params.q;
  const customerId = req.customerId;

  try {
    const userHasRating = await Rating.find({ user_id: customerId });
    if (
      userHasRating == null ||
      ...
      const recommend = await axios.get(
        `http://localhost:5500/?q=${parseInt(customerId) - 1}`
      );
...
router.get("/", async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: "Interval  test server error" });
  }
});
...
// @access Private
router.get("/rating/not-rating", verifyCustomerToken, async (req, res, next) => {
  try {
...
router.get("/admin/private", verifyToken, async (req, res) => {
  try {
...
// @access Private
router.post("/filter/:query", async (req, res) => {
  try {
