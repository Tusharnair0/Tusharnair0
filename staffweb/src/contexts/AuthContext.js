...
import { apiUrl, LOCAL_STORAGE_TOKEN_NAME, LOCAL_STORAGE_STAFF_NAME } from "./constant";
...
const AuthContextProvider = ({ children }) => {
  ...
  const loadUser = async () => {
    if (localStorage[LOCAL_STORAGE_STAFF_NAME]) {
      setAuthToken(localStorage[LOCAL_STORAGE_STAFF_NAME]);
    }
    try {
      ...
      if (response.data.success) {
        dispatch({
          type: "SET_AUTH",
          payload: { isAuthenticated: true, user: response.data.staff },
        });
      }
    } catch (error) {
      localStorage.removeItem(LOCAL_STORAGE_STAFF_NAME);
      setAuthToken(null);
      dispatch({
        type: "SET_AUTH",
  ...
  const loginUser = async (userForm) => {
    try {
      const response = await axios.post(`${apiUrl}/staff/login`, userForm);
      if (response.data.success)
        localStorage.setItem(
          LOCAL_STORAGE_STAFF_NAME,
