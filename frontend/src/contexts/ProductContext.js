...
const ProductContextProvider = ({ children }) => {
  ...
  const loadProduct = async () => {
    try {
      let response = await axios.get(`${apiUrl}/products/admin/private`);
      dispatch({
      ...
      dispatch({
        type: PRODUCT_REDUCER_PRODUCT,
        payload: { productLoading: true, product: [] },
      });
    }
  };
  ...
  const getProducts = async () => {
    try {
      const response = await axios.get(`${apiUrl}/products/admin/private`);
  ...
  const updateProducts = async (updateProduct) => {
    try {
      ...
      formData.append("categories", updateProduct.categories);
  ...
  const updateOutProducts = async (updateProduct) => {
    console.log("IN2");
    try {
      const { title, description, price, categories } = updateProduct;
